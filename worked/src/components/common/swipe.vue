      ...
      doAnimate(towards, options) {
        ...
        var callback = function () {
          if (newIndex !== undefined) {
            var newPage = that.$children[newIndex].$el;
        ...
        setTimeout(function () {
          if (towards === 'next') {
            that.translate(currentPage, -pageWidth, speed, callback);
            if (nextPage) {
              that.translate(nextPage, 0, speed);
            }
          } else if (towards === 'prev') {
            that.translate(currentPage, pageWidth, speed, callback);
            if (prevPage) {
              that.translate(prevPage, 0, speed);
            }
          } else {
            that.translate(currentPage, 0, speed, callback);
            if (typeof offsetLeft !== 'undefined') {
              if (prevPage && offsetLeft > 0) {
                that.translate(prevPage, pageWidth * -1, speed);
              }
              if (nextPage && offsetLeft < 0) {
                that.translate(nextPage, pageWidth, speed);
              }
            } else {
              if (prevPage) {
                that.translate(prevPage, pageWidth * -1, speed);
    ...
    ready() {
      ...
      if (that.auto > 0) {
        this.timer = setInterval(function () {
          if (!that.dragging && !that.animating) {
            that.next();
            console.info('next');
          }
        }, that.auto);
